...
function toBase64(bytes) {
  ...
  for (let i2 = 0;i2 < len2; i2 += 3) {
    const b1 = bytes[i2];
    const b2 = i2 + 1 < len2 ? bytes[i2 + 1] : 0;
    const b3 = i2 + 2 < len2 ? bytes[i2 + 2] : 0;
    result += BASE64_CHARS[b1 >> 2];
    result += BASE64_CHARS[(b1 & 3) << 4 | b2 >> 4];
    result += i2 + 1 < len2 ? BASE64_CHARS[(b2 & 15) << 2 | b3 >> 6] : "=";
    result += i2 + 2 < len2 ? BASE64_CHARS[b3 & 63] : "=";
  }
  return result;
}
...
class LLMService {
  ...
  sendRequest(runtime2, payload) {
    const httpClient = new ClientCapability3;
    const apiKey = this.apiKey;
    const baseUrl = this.baseUrl;
    const fetchAndParse = (sendRequester) => {
      try {
    ...
    try {
      const result = httpClient.sendRequest(runtime2, fetchAndParse, consensusIdenticalAggregation())().result();
      try {
        const parsed = JSON.parse(result);
        if (parsed.error) {
          runtime2.log(`LLM error: ${parsed.error}`);
          return null;
        }
      } catch {}
      return result;
    } catch (error) {
      runtime2.log(`sendRequest error: ${error.message || error}`);
      return null;
    }
  }
  getPredictionDecision(runtime2, payload) {
    const response = this.sendRequest(runtime2, payload);
    if (!response) {
      return null;
    }
    try {
      ...
      runtime2.log(`Failed to parse prediction decision: ${error.message || error}`);
      return null;
    }
  }
  sendToServer(runtime2, serverUrl, data, headers) {
    const httpClient = new ClientCapability3;
    const fetchAndParse = (sendRequester) => {
      try {
    ...
    try {
      const resultJson = httpClient.sendRequest(runtime2, fetchAndParse, consensusIdenticalAggregation())().result();
      return JSON.parse(resultJson);
    } catch (error) {
      runtime2.log(`sendToServer error: ${error.message || error}`);
...
var onHttpTrigger = (runtime2, payload) => {
  const requestPayload = decodeJson(payload.input);
  runtime2.log(`Received LLM request for model: ${requestPayload.model}`);
  if (!requestPayload.model || !requestPayload.messages) {
    runtime2.log(`Missing required data: model or messages`);
    return JSON.stringify({ error: "Missing required data: model and messages are required" });
  }
  const llmService = new LLMService(runtime2.config.openRouterApiKey ?? "");
  runtime2.log(`Forwarding request to LLM...`);
  const response = llmService.sendRequest(runtime2, requestPayload);
  if (!response) {
    runtime2.log(`Failed to get response from LLM`);
    return JSON.stringify({ error: "Failed to get response from LLM" });
  }
  runtime2.log(`Received response from LLM`);
  let parsedResponse = null;
  try {
    parsedResponse = JSON.parse(response);
    console.log(parsedResponse);
    if (parsedResponse.trade_decisions) {
      runtime2.log(`Received ${parsedResponse.trade_decisions.length} trading decisions`);
      for (const decision of parsedResponse.trade_decisions) {
        runtime2.log(`${decision.asset}: ${decision.action} - $${decision.allocation_usd}`);
      }
    }
  } catch {
    parsedResponse = { raw_response: response };
  }
  const callbackUrl = runtime2.config.callbackUrl;
  if (callbackUrl) {
    runtime2.log(`Sending response to callback server: ${callbackUrl}`);
    const serverResult = llmService.sendToServer(runtime2, callbackUrl, {
      llm_response: parsedResponse,
      timestamp: runtime2.now().toISOString()
    });
    if (serverResult.success) {
      runtime2.log(`Successfully sent to server`);
    } else {
      runtime2.log(`Failed to send to server: ${serverResult.error}`);
    }
...
var initWorkflow = (config) => {
  ...
  return [
    ...
    cre.handler(httpCapability.trigger({
      authorizedKeys: [
        {
